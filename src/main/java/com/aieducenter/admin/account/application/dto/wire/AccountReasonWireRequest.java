...
/**
 * 账号管理写出站请求体——identity {@code DisableAccountCommand} / {@code ManagementReasonCommand}
 * 的 wire 镜像，四个管理写端点（disable / activate / unlock / sessions-revoke）共用。
 *
 * <p>四个端点的 identity 请求体形状一致——都是 {@code {reason}}：封号 reason 必填（由北向
 * {@link com.aieducenter.admin.account.application.dto.command.DisableAccountCommand} 的 {@code @NotBlank}
 * 兜）、解封/解锁/强制下线 reason 可空（透传 null）。故共用一个 wire 记录，必填/可空的差异在北向 command 层校验。</p>
 *
 ...
 *
 * @param reason 操作原因（disable 必填、activate/unlock/sessions-revoke 可空）
 * @since 0.1.0
 */
