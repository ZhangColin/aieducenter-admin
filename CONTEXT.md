...
**AI 平台管理上下文 (Aiplatform Admin Context)**:
admin 内 `aiplatform` 子包承载的 **AI 开发平台（aiplatform）运营监管**关注点——订单报价/取消/归档、项目与交付物查阅、沙箱治理、平台成本、知识素材治理、单价表维护（spec #62：六域 34 端点忠实透传）。admin 作为 BFF 经 cartisan-openapi 签名调用 aiplatform `/api/backoffice/**`（机机签名，全局单 key `admin-console`）。与 payment/account admin 同构（BFF 纯透传、操作者身份走 RequestContext 落 aiplatform 审计列、出站客户端裸 `@Component` [ADR-0007](docs/adr/0007-admin-bff-outbound-clients-are-flat-components.md)），两处**有意的差异**：① 错误**原样透传不做映射**——aiplatform 错误信封 `code` 装数字业务码（域码×1000＋序号，如 IDN_004→6004），HTTP 状态独立，`AiplatformClient` 统一翻译为 `AiplatformUpstreamException`（六域复用、AppService 不再 try/catch），`AiplatformUpstreamErrorAdvice` 北向还原 provider 信封（区别于 payment/account 的 `BaseCodeMessage` 映射式翻译——映射会杀前端比对 aiplatform 业务码的分支，aiplatform #169 教训）；② 北向分页按平台新决议**目标态**实现：请求 1-based + 回显 1-based，全程无 ±1 换算（aiplatform wire 本就 1-based，平台分页统一 2026-09-15 拍板）。成本域另两处口径（issue #67）：时间窗 `from/to`（半开 `[from, to)`、ISO-8601 Instant UTC）北向**必填**——provider 可缺省（缺省＝该侧不限），BFF 不设默认窗口、不暴露 provider 的不限能力（防无界扫描）；计量出口 `TokenUsage` 五档（input/output/cacheRead/cacheWrite/reasoning）为 primitive long——provider 出 JSON 数字（cartisan-web 全局 Long→ToStringSerializer 只覆盖包装类型），BFF 同形镜像。权限码每域读码独立 + 每写操作独立（`admin:aiplatform:{domain}:read` / 逐写操作）；账号读口独立码 `admin:aiplatform:account:read`（无独立页面，订单/项目详情抽屉嵌用）。provider「开行」端点不暴露（种子脚本通道，web 改价只走原子 reprice）。
_Avoid_: 把 aiplatform 错误映射成 `BaseCodeMessage`（NOT_FOUND 等——丢数字业务码）；在 admin 侧镜像 provider 错误码目录/为 `@ErrorCodes` 注册枚举（漂移面，框架 validator 要求码在本地注册）；把操作者（admin 管理员）与 aiplatform accountId/externalId 混读。
...
**BFF 出口枚举须配 *Name 中文名 (BFF enum responses carry display names)**:
...
平台统一——凡 `PageResponse{items,total,page,size}` 外壳：**全链 1-based**（请求与响应同语义，第 1 页是 1）；±1 换算**唯一收 cartisan-boot**（`Pagination.toPageRequest()` 请求 1-based→Spring 0-based；`PageResponse.of()` 回显 0-based→1-based），本仓主代码**零手工换算**。北向列表端点统一框架 `Pagination` 绑定（page 缺省 1、`page<1` 静默贴边 1；size 缺省 20、上限 100、越界贴边；sort 走框架 `toPageRequest(...)` 出口）。本仓两种出口模式：自有域 `Pagination` + `PageResponse.of(page.map(...))`；BFF 域 wire page 与北向同值直传 `*Client`（无 ±1）+ 透传下游回显（迁移后各服务回显==请求页码）。aiplatform 域保持既有 `@RequestParam(defaultValue = "1")` 形态（wire 等价，不重构）。造 mock/fixture 时 `page` 按「==请求页码」构造。见 [ADR-0012](docs/adr/0012-pagination-full-chain-1-based.md)（supersede ADR-0010，[#73](https://github.com/ZhangColin/aieducenter-admin/issues/73)）。横切规则，兄弟 BFF 子上下文与能力域新列表端点继承。
_Avoid_: 主代码出现任何 `page - 1` / `getPageNumber() + 1`（换算归框架）；BFF 响应构造对下游回显再 +1（2-based 回归）；北向新列表端点用 Spring `Pageable` 直绑或 `@RequestParam` 手接分页（应统一 `Pagination`）；mock 建模下游回显用 wire 裸页码或旧「页码+1」；验证下游回显语义时拿本仓 mock 当证据（以兄弟仓服务端源码为准）。
